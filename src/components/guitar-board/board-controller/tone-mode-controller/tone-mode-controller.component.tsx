...
export const ToneModeController = () => {
	...
	const handleClick = (item: ModeConfigType) => {
		guitar.setOptions({ mode: item.key })
	}
	...
	return (
		<ControllerList
			...
			checkedItem={(item) => item.key === guitar.board.mode}
            visibleItem={() => true}
			renderListItem={renderInstrumentItem}
            className={cx(styles['container'], componentStyles['mode-container'])}
		/>
	)
}
