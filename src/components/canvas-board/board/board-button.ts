import { Point, ToneTypeName } from 'to-guitar'
import { Color, GRADE_LINE_WIDTH, SINGLE_HEIGHT, SINGLE_WIDTH } from './config.js'
	...
	implements BoardButtonOptions
{
	...
	draw = () => {
		...
		// 按钮
		this.ctx.fillStyle = checked ? buttonCheckedColor : focus ? buttonFocusColor : buttonColor
		this.ctx.fill(this.drawable.button)

		// 和弦字符
		this.ctx.fillStyle = onlyButton ? lineColor : textColor
		this.ctx.font = `${r}px not specified`
		this.ctx.fillText(this.point.toneSchema[ToneTypeName], this.x, this.y)
	}
