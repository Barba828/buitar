import { Point, transBoard } from 'to-guitar'
...
export class Board extends Drawable<any> implements BoardOptions {
		...
		strings.forEach((string, xIndex) => {
			string.forEach((point, yIndex) => {
				...
				if (yIndex === 0) {
					...
					button = new BoardButton(this.ctx, x, y, point, {
						pointDisable: point.toneSchema.interval.includes('#'),
					})
				}
	...
	addButtonListener = (type: 'mousemove' | 'click' | 'change', listener: (button: BoardButton) => void) => {
		if (type === 'mousemove') {
			...
			this.canvas.addEventListener('click', (e) => {
				const button = this.boardButtons.find((button) => button.isOver(e.offsetX, e.offsetY))
				if (!button) {
					return
				}
	...
	drawSetOptions = (allButton: boolean) => {
		this.boardButtons.forEach((button) => {
			if (!allButton) {
				button.setOptions({
					pointDisable: button.point.toneSchema.interval.includes('#'),
				})
			} else {
	...
	drawSetOptions2 = (chord: Point[]) => {
		...
		this.boardButtons.forEach((button) => {
			...
			const buttonColor = checked ? Color.Button.Checked : Color.Button.Default
